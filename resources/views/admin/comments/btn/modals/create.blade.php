            ...
            <div class="modal-body">
                <form action="{{ route('comment.store') }}" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" required name="body" id="body" value="">
                        @error('body')
                        <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="p-1 form-group">
                        ...
                        <select name="status" class="form-control">
                            ...
                            <option value="0" {{ old('type') == '0' ? 'selected' : '' }}>Not Active</option>
                            <option value="1" {{ old('type') == '1' ? 'selected' : '' }}>Active</option>
                        </select>
                        @error('status')
                            <div class="alert alert-danger">{{ $message }}</div>
                        @enderror
