<!-- Modal -->
<div class="service-modal modal fade" id="serviceModal{{$order->id}}" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="modalForm" action="{{route('website.free-order.newUpdate', $order->id)}}" method="post">
                    @csrf
                    @method('PUT')
                    <div class="modal-body selection service-modal-body">
                        @foreach ($services as $service)
                        <label class="d-block {{ $service->name }}-cont">
                            <input type="checkbox" name="order_service_id[]" value="{{$service->id}}"
                                data-target="{{ $service->name }}-cont" data-service="{{ $service->id }}">
                            {{ $service->name }}
