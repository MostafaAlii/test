...
use App\DataTables\CoupleDataTable;
use App\Models\Couple;
use Illuminate\Support\Facades\{DB, Session};
class CoupleController extends Controller {
    public function index(CoupleDataTable $couple) {
        return $couple->render('admin.couples.index', ['title' => 'Couples']);
    }

    public function store(Request $request) {
        try {
            $orderService = Couple::create($request->input());
    ...
    public function update(Request $request, $id) {
        try {
            $orderService = Couple::findorfail($id);
